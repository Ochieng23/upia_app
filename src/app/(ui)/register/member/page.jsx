...
function Step2OTPRequest({ eligibility, onOTPSent }) {
  ...
  useEffect(() => {
    fetch(`${BASE}/ippms/counties`)
      ...
      .then((d) => {
        const list = d.data || []
        if (list.length === 0) setLocationError('Could not load counties. Please refresh the page.')
        setCounties(list)
      })
      .catch(() => setLocationError('Could not load counties. Please check your connection and refresh.'))
  }, [])
  ...
  const onCountyChange = async (code) => {
    ...
    try {
      ...
      setLocationError('Could not load constituencies. Please try again.')
    }
  }
  ...
  const onConstituencyChange = async (code) => {
    ...
    try {
      ...
      setLocationError('Could not load wards. Please try again.')
    }
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!data.registrationId) throw new Error('No registration ID returned. Please try again.')
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {locationError && (
        <div className="rounded-[6px] border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700">{locationError}</div>
      )}
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main className="mx-auto max-w-2xl px-4 sm:px-6 lg:px-8 py-14">
        <Link href="/register" className="inline-flex items-center gap-1.5 text-sm text-[#5A5450] hover:text-[#111111] mb-8 transition-colors">
